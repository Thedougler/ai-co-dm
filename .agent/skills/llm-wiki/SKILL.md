...
name: llm-wiki
description: >
  Foundational Karpathy LLM-wiki pattern for ai-co-dm. Use for ingest/query/lint doctrine,
  three-layer architecture (raw → wiki → schema), index/hot, provenance, and when agents
  debate how to grow the vault. Operations: qmd-retrieval for find; Organizer for hygiene;
  AGENTS.md is schema. Do not create a parallel wiki/ or concepts/ tree in this repo.
---
...
This vault is already an LLM wiki. **Do not scaffold a second `wiki/` · `sources/` · `concepts/` tree.**

| Generic llm-wiki | ai-co-dm |
|---|---|
| Schema | [[AGENTS]] + `templates/` |
| Compiled wiki | `campaigns/`, `lexicon/`, hubs, indexes |
| Hot / index | `campaigns/<id>/hot.md`, `00 Home.md`, typed `00 *.md` indexes |
| Raw / inbox | `inbox/` (capture→file only) |
| Query | `./scripts/qmd` + skill `qmd-retrieval` |
| Lint / hygiene | **Organizer** (`wiki-health` routine) |
| Ingest | Co-DM / Organizer filing; skills for design; no parallel DB |

Progressive load: this `SKILL.md` for pattern; `references/karpathy-pattern.md` only when teaching the pattern. Day-to-day find/write stays on AGENTS + qmd-retrieval.
...
## This repository: Campaign OS

**Campaign OS is the llm-wiki for campaigns.** Same three layers; this vault's schema is Campaign OS, not generic `concepts/` · `entities/` · `projects/`.

| Layer | Here |
|---|---|
| Raw | `_raw/`, `OBSIDIAN_SOURCES_DIR`, publications, session evidence |
| Wiki | `OBSIDIAN_VAULT_PATH` → `wiki/`; pages are `kind.slug.md` |
| Schema | `CONTEXT.md`, ADRs, `_system/schemas/`, `AGENTS.md` |

Page contract: `id`, `kind`, `scope`, `canon` (plus kind fields). Semantic layers: `world/` · `campaign/` · `rules/` · `sessions/` · `sources/` · `assets/`. Ingest always starts when invoked and mints every decomposition node (*stub* if thin); see `wiki-ingest`.

When sections below illustrate generic categories, `title`/`category` frontmatter, `projects/`, or importance `tier`/`lifecycle` fields, **map to Campaign OS** for this vault:

| Generic illustration | Campaign OS |
|---|---|
| `concepts/` `entities/` `skills/` `references/` … | kind → schema `directory` + `scope` |
| `title` / `category` | `id` / `kind` (+ `aliases` for display) |
| `projects/<name>/…` | not used; campaign-specific truth uses `scope: campaign` |
| `tier: core\|supporting\|peripheral` | `importance` + `status` |
| Ledger paths at "vault root" | page ledgers under `wiki/` (`.manifest.json`, `index.md`, `log.md`, `hot.md`); operational ledgers under `$REPO_ROOT/_system/state/` |

Operational protocol in this skill still applies: config resolution, provenance markers, link format, typed relationships, QMD refresh primitives.

## Campaign OS roots

Campaign OS has two explicit roots. `$OBSIDIAN_VAULT_PATH` is the Obsidian vault
(`wiki/` here) and owns canonical pages, vault ledgers, assets, and vault UI or
renderer files. `$REPO_ROOT` is the repository root and owns `_system/`, `_raw/`,
and `_archive/`; compiler state and configuration never belong under
`$OBSIDIAN_VAULT_PATH/_system/`.

Resolve the repository root only when the resolved config exposes the existing
`OBSIDIAN_SCHEMA_SOURCE` authority locator:

```bash
REPO_ROOT="$(cd "$(dirname "$OBSIDIAN_SCHEMA_SOURCE")" && pwd -P)"
SYSTEM_ROOT="$REPO_ROOT/_system"
```

In this repository `OBSIDIAN_SCHEMA_SOURCE` points to the repository's
`AGENTS.md`, so this derivation is deterministic. Do not infer a repository
root from the parent of `$OBSIDIAN_VAULT_PATH` for a portable standalone vault.
When `OBSIDIAN_SCHEMA_SOURCE` is absent or does not identify the Campaign OS
authority, keep generic operational files in the host wiki tool's configured
location instead of inventing a vault-local `_system/` tree.
...
For Campaign OS, deterministic `wiki-lint` validates an approved trust ledger
only when a real ledger is configured and exists. This repository currently has
no trust ledger, so report trust review as unavailable rather than inferring or
creating a ledger path. Generic installations retain the host wiki tool's
configured ledger location. The validator does not recompute confidence from
source strings. New or materially changed pages are marked for manual review.
Refresh a ledger only after explicit human approval.
...
The `tier:` field controls update priority and retrieval order. As wikis grow,
re-reading every unrelated page on every ingest wastes tokens. It never permits
an ingest to omit a source claim or to avoid a complete read of a wiki file that
`wiki-ingest` classifies as relevant.
...
| `peripheral` | Low-connectivity pages — rarely linked, narrowly scoped | Lower update priority when the source is not relevant | Last resort; skipped when trimming to context budget |
...
- `wiki-ingest` may use `tier:` to order candidate review, but its READ → SEARCH contract
  requires a complete read for every file classified as relevant
...
Locate pages by meaning with QMD, then read the hits. `wiki-ingest` uses snippets only to route SEARCH candidates; once a file is relevant, it reads that file completely before RECONCILE. Commands live in `AGENTS.md`; syntax from `qmd skill show`.

| Need | Primitive |
|---|---|
| Find pages about a subject | `qmd query` / `qmd search` `-c wiki` |
| Intake under `_raw/` | same with `-c raw` |
| Retired pages | same with `-c archive` |
| A claim inside a known file | `Grep -A <n> -B <n> "<term>" <file>` |
| All `[[wikilinks]]` or one frontmatter field across the vault | Grep |
| TypeScript, schemas, skills under `_system/` | Grep or LSP |
| Whole-page content | `Read` / `qmd get --full-path` |

Capitalized `Grep`/`Glob` names in these skills are tool-generic primitives. Skills that consume this table: `wiki-query`, `cross-linker`, `wiki-lint`, `wiki-status` (insights mode).
...
- `OBSIDIAN_RAW_DIR` — Raw staging directory; defaults to `$OBSIDIAN_VAULT_PATH/_raw` when unset
- `OBSIDIAN_SOURCES_DIR` — Where raw source documents are
- `OBSIDIAN_SCHEMA_SOURCE` — Schema/owner authority locator (in Campaign OS, the repository's `AGENTS.md`; its parent derives `$REPO_ROOT`)
