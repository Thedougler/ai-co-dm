---
type: location
campaign: shattered-sea
kind: vehicle
region: unknown
visibility: dm
tags: [location]
---
# Saltwright
> [!narration] Narration
> _Visualizer: place portrait pending._
## At a glance
Vessel that carried the party through a Dravosi boarding fight; feels like a remembered escape route.
## Five-sentence kernel
- **Function:** A vessel tied to the party’s escape, smuggling, and defector thread.
- **Fantastic:** A remembered boarding route remains a usable political and spatial clue.
- **Conflict:** smugglers, Dravosi authority, defectors, and the ship’s future claim different routes.
- **Promise:** Reconstruct a boarding, identify who helped, and decide what the escape created.
- **Trajectory:** its contacts are hunted, its route is exposed, or it becomes leverage for Passage.
## 3Fs and contradiction
- **Fantastic:** a vessel is both escape route and evidence.
- **Familiar:** hull, deck, cargo, lines, boarding interface.
- **Functional:** smuggling and defectors use its movement and contacts.
- **Contradiction:** escape made the crew visible to new powers.
- **Signatures:** boarding scars; hidden cargo space (**seed); names tied to three contacts.
## Current situation
- **Canon:** carried party through Dravosi boarding fight; smuggling, boarding action, defectors; links Beaumont Sel, Barnaby Rook, Geoffrey Draves.
- **DM seed (unconfirmed):** exact ship plan, current location, and ownership are open.
## Aspects
`boarding scar, smuggler route, defector network`
## Vehicle surface and topology
- `water -- approach (visible route, recognition risk) --> Saltwright`.
- `deck -- boarding interface (cover and movement) --> cargo/crew space` (**exact layout seed**).
- `cargo -- hidden route (privacy, evidence) --> defector thread` (**seed**).
- `deck -- retreat/launch --> open water or contact port`.
- **Loop:** approach → deck → cargo → contact route → water.
- **Bypass:** meet Beaumont/Barnaby/Geoffrey away from vessel; loses physical evidence.
## Player verbs
`board, search, reconstruct, question, hide, signal, repair, expose, choose a contact`.
## Who is here
- [[Beaumont Sel]], [[Barnaby Rook]], [[Geoffrey Draves]] as linked contacts; current roster **DM seed (unconfirmed)**.
## Information economy
- **Secret:** who benefited from the boarding and what defector route remains.
- **Witness:** one linked contact’s account.
- **Trace:** boarding damage, cargo tie, route mark (**seed**).
- **Document:** Crown record or smuggling note (**seed**).
- **Failure cost:** contact trust, exposure, or lost route; escape clue remains.
## Pressure and moves
- **Pressure:** remembered → watched → compromised → repurposed.
- **Move:** Crown identifies a route after a public sighting; papers/patrols change; Passage can offer a new crossing; old route is no longer private.
- **Move:** a defector changes position after contact pressure; visible absence or new signal; follow or protect; relationship persists.
## Connections
- [[Beaumont Sel]] · [[Barnaby Rook]] · [[Geoffrey Draves]]
## Hooks
- Revisit the boarding, protect defectors, and connect Saltwright’s route to wider Crown/Passage pressure.
## Secrets (DM)
- Current location, ship’s build, crew, and hidden cargo are **DM seed (unconfirmed)**.
## Mechanics and return
- Vehicle boarding uses cover, movement, and Search/Study/Influence/Utilize; do not invent numeric ship stats. Failure costs position/resources/trust.
- **Return state:** contact status, route exposure, physical evidence, and whether Saltwright remains a safe handoff.
