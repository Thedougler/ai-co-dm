...
- **Foundry staging:** when Nick asks to create, push, or stage an entity in FoundryVTT, load `foundry-stage` and use the configured `foundry-mcp` MCP server from `mcp.json` for world CRUD. Follow `check → stage → wire → verify`: check with `list-characters`/`manage-world-items`/`list-scenes`; create 5.5e NPCs with `dnd5e-create-npc`; wire SRD matches with `dnd5e-add-features-from-compendium`, then remaining traits/actions with `dnd5e-add-feature`; verify with `get-character` and `get-character-entity` readback. The create-NPC call does not add features or token art.
- **Foundry assets:** copy the exact owner token from `attachments/` to `foundry-data/assets/` and pass Foundry the `assets/...` reference, never a vault or disk path. For an actor token, update the actor image through `manage-actors` and confirm the persisted actor image; place scene tokens only when placement is explicitly requested. Never edit Foundry LevelDB/data files directly, and runtime staging does not authorize Markdown changes.
