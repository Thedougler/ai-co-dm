...
9. **Obsidian markdown** — run `./scripts/lint-obsidian-markdown`. FAIL = in-vault markdown links, DC-in-narration, forbidden `wiki/`·`concepts/`·`sources/`. Warn = title-only / missing frontmatter (`--strict` to fail). Skill: `obsidian-markdown`. Sample recent mechanical notes for at-table scan: `**Wisdom (Perception) — \`DC 15\`**` is the current mark; `**DC 15** *Perception*` is the old one. Report only; do not mass-rewrite the corpus.
